...
//#pragma omp for 
		for (frame_number = 0; frame_number < end_frame; frame_number++) {
			frame_rgb = NULL;
			loadImage(frame_number, image_path, &frame_rgb); // Loop order
			...
			convertRGBtoYCbCr(frame_rgb, frame_ycbcr); // Loop order
			...
			Channel* frame_downsampled_cb = downSample(frame_lowpassed_final->Cb); // Loop order
			frame_downsampled->Cb->copy(frame_downsampled_cb);
			Channel* frame_downsampled_cr = downSample(frame_lowpassed_final->Cr); // Loop order
...
int main(int args, char** argv){
    ...
    encode();
	gettimeofday(&endtime, NULL);
	runtime = double(endtime.tv_sec)*1000.0f + double(endtime.tv_usec) / 1000.0f - double(starttime.tv_sec)*1000.0f - double(starttime.tv_usec) / 1000.0f; //in ms  
	printf("\n\nTotal time = %f", runtime);
	return 0;
}
