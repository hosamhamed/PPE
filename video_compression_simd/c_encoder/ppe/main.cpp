...
#ifdef USE_SIMD
	__m256 r_mul_0 = _mm256_set1_ps(0.299);
	__m256 r_mul_1 = _mm256_set1_ps(-0.168736);
	__m256 r_mul_2 = _mm256_set1_ps(0.5);
	// green factors
	__m256 g_mul_0 = _mm256_set1_ps(0.587);
	__m256 g_mul_1 = _mm256_set1_ps(-0.331264);
	__m256 g_mul_2 = _mm256_set1_ps(-0.418688);

	// blue factors
	__m256 b_mul_0 = _mm256_set1_ps(0.113);
	__m256 b_mul_1 = _mm256_set1_ps(0.5);
	__m256 b_mul_2 = _mm256_set1_ps(-0.081312);
	
	// addition part
	__m256 part_add = _mm256_set1_ps(128.0f);

	for(int y=0; y<width; y+=8) {
		for (int x = 0; x<height; x++) {
			__m256 reg_r = _mm256_load_ps(&(in->rc->data[x*width + y]));
			__m256 reg_g = _mm256_load_ps(&(in->gc->data[x*width + y]));
			__m256 reg_b = _mm256_load_ps(&(in->bc->data[x*width + y]));

			__m256 reg_y = _mm256_add_ps(_mm256_mul_ps(r_mul_0, reg_r), _mm256_mul_ps(g_mul_0, reg_g));
			reg_y = _mm256_add_ps(_mm256_mul_ps(b_mul_0, reg_b),reg_y);


			__m256 reg_cb = _mm256_add_ps(_mm256_mul_ps(r_mul_1, reg_r), _mm256_mul_ps(g_mul_1, reg_g));
			reg_cb = _mm256_add_ps(_mm256_mul_ps(b_mul_1, reg_b), reg_cb);
			reg_cb = _mm256_add_ps(part_add, reg_cb);



			__m256 reg_cr = _mm256_add_ps(_mm256_mul_ps(r_mul_2, reg_r), _mm256_mul_ps(g_mul_2, reg_g));
			reg_cr = _mm256_add_ps(_mm256_mul_ps(b_mul_2, reg_b), reg_cr);
			reg_cr = _mm256_add_ps(part_add, reg_cr);

			_mm256_store_ps(&(out->rc->data[x*width + y]), reg_y);
			_mm256_store_ps(&(out->gc->data[x*width + y]), reg_cb);
			_mm256_store_ps(&(out->bc->data[x*width + y]), reg_cr);
		}
    }
#else
